...
namespace DataProcessor.Decoders.Tests
{
    ...
    public class IntegerDecoderTest
    {
        ...
        public void Decode_Given_that_property_pattern_is_not_set_Should_throw_an_exception()
        {
            ...
            target = new IntegerDecoder { FailValidationResult = ValidationResultType.InvalidWarning };
        ...
        {
            var field = new Field { Raw = "10" };
            target = new IntegerDecoder { Pattern = @"(?!0+)-?[0-9]{1,6}", FailValidationResult = ValidationResultType.InvalidWarning };
