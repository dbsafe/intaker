...
namespace DataProcessor.Decoders.Tests
{
    ...
    public class DecimalDecoderTest
    {
        ...
        public void Decode_Given_that_property_pattern_is_not_set_Should_throw_an_exception()
        {
            ...
            target = new DecimalDecoder { FailValidationResult = ValidationResultType.InvalidWarning };
        ...
        {
            var field = new Field { Raw = "10.05" };
            target = new DecimalDecoder { Pattern = @"(?!0+)-?[0-9]{1,6}\.[0-9]{2}", FailValidationResult = ValidationResultType.InvalidWarning };
